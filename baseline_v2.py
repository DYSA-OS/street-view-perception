...
def create_data_generator(df, class_name, batch_size=1, target_size=(224, 224), shuffle=False):
    df['img_path'] = df['filename'].apply(lambda val: os.path.join('./datasets', val))
...
def training(class_name, result_dir):
    train_df = pd.read_csv(f'./balanced_data/origin_train_{class_name}.csv')
    val_df = pd.read_csv(f'./balanced_data/origin_val_{class_name}.csv')
    test_df = pd.read_csv(f'./balanced_data/origin_test_{class_name}.csv')
    ...
    with open('./balanced_results/test.txt', 'a') as f:
...
def main():
    result_dir = './balanced_results'
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)

    baseline_dir = './balanced_results/baseline'
    ...
    for class_name in classes:
        training(class_name, baseline_dir)
